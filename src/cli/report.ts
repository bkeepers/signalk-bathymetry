...
export async function run() {
  const { values } = parseArgs({
    options: {
      config: { type: 'string', short: 'c', default: join(homedir(), ".signalk", "plugin-config-data", "bathymetry.json") },
  ...
  function getConfig() {
    const { configuration } = JSON.parse(readFileSync(values.config).toString());
